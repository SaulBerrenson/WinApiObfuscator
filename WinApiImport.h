...
public:
	...
	WinApiImport(const std::string& func_name, const std::string& module_name, unsigned seed = 0)
		: m_func_name(func_name),
		m_module_name(module_name)
	{
		m_len = func_name.length();
	...
	std::function<T> get_function() {
		try
		{
			HMODULE krnl32, hDll;
			LPVOID api_func;
			DWORD api_hash = murmur_hash2_a(m_func_name.c_str(), m_len, m_seed);
#ifdef _WIN64
